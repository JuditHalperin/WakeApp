...
def main():
    ...
    root.wm_title("Title")
    root.wm_protocol("WM_DELETE_WINDOW", on_close)  # when X is clicked

    stop_button = tk.Button(root, text="Stop", command=on_close)
    stop_button.pack(side="bottom", fill="both", expand="yes", padx=10, pady=10)

    thread = Thread(target=video_loop(vs))
    thread.start()
